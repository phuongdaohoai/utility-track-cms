/**
 * Login Page
 * Redesigned to match provided layout (split-screen, centered login card)
 */
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="flex items-center justify-center bg-white">
         {/* Language switch top-right (moved inside relative container so buttons are clickable) */}
          <div className="absolute top-[5%] right-[5%] z-20 font-bold flex items-center gap-6 text-sm text-gray-500">
            <button
              type="button"
              onClick={() => setLocale('vi')}
              aria-pressed={locale === 'vi'}
              className={`focus:outline-none ${locale === 'vi' ? 'underline decoration-2 underline-offset-4 text-indigo-700' : 'text-gray-400 hover:text-indigo-600'}`}
            >
              Tiếng Việt
            </button>
            ...
            <button
              ...
              className={`focus:outline-none ${locale === 'en' ? 'underline decoration-2 underline-offset-4 text-indigo-700' : 'text-gray-400 hover:text-indigo-600'}`}
            >
              English
            </button>
          </div>
        <div className="relative max-w-md w-full px-6 py-12">
          <div className="bg-white border rounded-2xl  p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label htmlFor="email" className="block font-bold text-sm text-gray-600 mb-1">Email hoặc Số điện thoại</label>
                <input
                  id="email"
                  type="email"
                  value={email}
              ...
              <div>
                <label htmlFor="password" className="block font-bold text-sm text-gray-600 mb-1">Mật khẩu</label>
              ...
              <div className="pt-2">
                <Button type="submit"  size="md" className="w-full bg-[#3a5a89]">Đăng Nhập</Button>
              </div>
            </form>
          </div>
